...
RSpec.describe RPNCalculator::Operation::Subtraction do
  ...
  context 'when the operands are valid' do
    let(:valid_elements) { [7.0, 3.0] }

    subject { described_class.new(valid_elements) }

    describe '#result' do
      it 'returns the sum of the operands' do
        expect(subject.result.result).to eq(4.0)
      end
    end

    describe '#.operate' do
      it 'rreturns the sum of the operands' do
        expect(described_class.operate(valid_elements).result).to eq(4.0)
      end
    end
  end
end
